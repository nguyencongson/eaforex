...
extern double Start_Trailing_At = 1.9;  // org 2.9 meaning keep bid/ask distance by x PIPS
...
int start() {
   ...
   if (!Gi_268) { // một biến nào đó
      for (int pos_108 = OrdersHistoryTotal() - 1; pos_108 >= 0; pos_108--) { // pos_106 là biến i
         if (OrderSelect(pos_108, SELECT_BY_POS, MODE_HISTORY)) { // lựa chọn lệnh
            if (OrderProfit() != 0.0) { // nếu lợi nhuận lệnh đso khác 0
               if (OrderClosePrice() != OrderOpenPrice()) { // nếu đóng cửa khác giá mở cửa
                  if (OrderSymbol() == Symbol()) { // nếu lệnh đó có giá trị tiền đúng với chart
                     Gi_268 = TRUE; // đưa biến đó thành true vì cơ bản biến đầu tiên là false
                     Ld_112 = MathAbs(OrderProfit() / (OrderClosePrice() - OrderOpenPrice())); // Lợi nhuận chia cho giá đóng trừ giá mở
                     Gd_272 = (-OrderCommission()) / Ld_112; // gán cho biến gd272 thành - commssion / giá trị ld_112
                     break;
                  }
               }
            }
   ...
   double Ld_120 = Ask - Bid; // biến ld 120 ask-bid = spread
   ArrayCopy(Gda_280, Gda_280, 0, 1, 29); // copy mảng gda_280 vào 29 giá trị
   Gda_280[29] = Ld_120; 
   if (Gi_284 < 30) Gi_284++;
   double Ld_128 = 0;
   pos_108 = 29;
