...
//Версия 2 по комментарию "stace" отсылка ордера в двух вариантах
//1. вариант флаг Two_Slash=0 - отсылка полного ордера.
//2. вариант Two_Slash=1 - отсылка 
//в два этапа - сначала ордер с tp/sl=0, затем модификация +tp +sl
//дополнена обработка ошибок, повторно не отсылать если бесполезно
//также исправлены неудоства с текстовой строкой, изменены имена глобальных переменных, чтобы в одно месте
  ...
  {if (twoslash==1)
         {
            ...
            if(ticket>0) 
               { //Sleep(1000); //Если брокер не дает быстро изменять, снять //, подобрать задержку
                  send_order(1,ticket,Lot,Slipp,Pop,Sell_tp,Sell_sl); //modify
               }
         }
         else send_order(0,ticket,Lot,Slipp,Pop,Sell_tp,Sell_sl); //send +tp/sl
   }
   ObjectDelete("PipsWork");
}
//....................................................
...
err=1; k=0;
         ...
         { k+=1;  RefreshRates();
                 ...
                 if(sm==0) ticket=OrderSend(Symbol(),OP_SELL,Lot,Bid,Slipp,loss,profit,NULL,0,0,CLR_NONE);                       
                 else      result=OrderModify(ticket,0,loss,profit,0,CLR_NONE);                
         err=ShowError(Pop,k);
         if(err==0&&result==TRUE) break; else { Sleep(1000);  }
         if(k>=Pop) break;
             //повтор только при 4, 6, 135, 136 137 138 145 146 
            switch(err)
            {  
...
{
   string description;
   int err=GetLastError(),crach; 
   switch (err)            //повтор только при 4, 6, 135, 136 137 138 145 146 
   {
      case   0: return;
